...
public class UR5Controller : MonoBehaviour
{
    ...
    public float x = 0.1f, y = 0.2f, z = 0.3f, phi = 10, theta = 10, psi = 10, oldX, oldY, oldZ, oldPhi, oldTheta, oldPsi;
    ...
    public Slider sliderUI_Joint0;
    public Slider sliderUI_Joint1;
    public Slider sliderUI_Joint2;
    public Slider sliderUI_Joint3;
    public Slider sliderUI_Joint4;
    public Slider sliderUI_Joint5;
    private float slider_Joint0_Value, slider_Joint1_Value, slider_Joint2_Value, slider_Joint3_Value, slider_Joint4_Value, slider_Joint5_Value;

    public GameObject CartesianMenu;
    public GameObject JointMenu;
    public GameObject CurrentState_JointSpace;
    public GameObject CurrentState_CartesianSpace;

    private int MenuSelector = 0;
    private int OneTime = 0;

    private string x_input = "0,1";
    private string psi_input = "50";
    private string y_input = "2,2";
    private string theta_input = "200";
    private string z_input = "3,1";
    private string phi_input = "-20";

    public Text x_txt;
    public Text y_text;
    public Text z_text;
    public Text psi_text;
    public Text theta_text;
    public Text phi_text;

    private int chet = 1;

    public Button OneToOne_Btn;
    public Slider LightIntensity;
    public Dropdown Shadow_Dpdwn;
    public Scrollbar Shadow_Strength;

    public Camera main_camera;
    public Toggle Orthographic_Toggle;


    //void Awake()
    //{
    //    OneToOne_Btn = GetComponent<Button>();
    ...
    void Start()
    {
        ...
        CurrentState_JointSpace.SetActive(true);
        CurrentState_CartesianSpace.SetActive(false);

        JointMenu.SetActive(true);
        CartesianMenu.SetActive(false);
    ...
    void LateUpdate()
    {
        ...
        slider_Joint0_Value = sliderUI_Joint0.value;
        slider_Joint1_Value = sliderUI_Joint1.value;
        slider_Joint2_Value = sliderUI_Joint2.value;
        slider_Joint3_Value = sliderUI_Joint3.value;
        slider_Joint4_Value = sliderUI_Joint4.value;
        slider_Joint5_Value = sliderUI_Joint5.value;

        if (MenuSelector == 0)
        {
            if (OneTime == 0)
            {
                sliderUI_Joint0.value = Robot11.solutionArray[0];
                sliderUI_Joint1.value = Robot11.solutionArray[1];
                sliderUI_Joint2.value = Robot11.solutionArray[2];
                sliderUI_Joint3.value = Robot11.solutionArray[3];
                sliderUI_Joint4.value = Robot11.solutionArray[4];
                sliderUI_Joint5.value = Robot11.solutionArray[5];
                OneTime = 1;
            }

            Vector3 currentRotation_joint0 = jointList[0].transform.localEulerAngles;
            currentRotation_joint0.z = slider_Joint0_Value;
            jointList[0].transform.localEulerAngles = currentRotation_joint0;

            Vector3 currentRotation_joint1 = jointList[1].transform.localEulerAngles;
            currentRotation_joint1.z = slider_Joint1_Value;
            jointList[1].transform.localEulerAngles = currentRotation_joint1;

            Vector3 currentRotation_joint2 = jointList[2].transform.localEulerAngles;
            currentRotation_joint2.z = slider_Joint2_Value;
            jointList[2].transform.localEulerAngles = currentRotation_joint2;

            Vector3 currentRotation_joint3 = jointList[3].transform.localEulerAngles;
            currentRotation_joint3.z = slider_Joint3_Value;
            jointList[3].transform.localEulerAngles = currentRotation_joint3;

            Vector3 currentRotation_joint4 = jointList[4].transform.localEulerAngles;
            currentRotation_joint4.z = slider_Joint4_Value;
            jointList[4].transform.localEulerAngles = currentRotation_joint4;

            Vector3 currentRotation_joint5 = jointList[5].transform.localEulerAngles;
            currentRotation_joint5.z = slider_Joint5_Value;
            jointList[5].transform.localEulerAngles = currentRotation_joint5;
        }

        if ((userHasHitOk) | (OneToONe))
        {
        ...
        y_text.text = y_input;
        z_text.text = z_input;
        psi_text.text = psi_input;
        theta_text.text = theta_input;
        phi_text.text = phi_input;

        Light myLight = GameObject.Find("MyLight").GetComponent<Light>();
        myLight.intensity = LightIntensity.value;
    }
    ...
    public void EnableCartesianMenu()
    {
        CartesianMenu.SetActive(true);
        JointMenu.SetActive(false);

        CurrentState_JointSpace.SetActive(false);
        CurrentState_CartesianSpace.SetActive(true);
        MenuSelector = 1;
    }


    public void EnableJointMenu()
    {
        JointMenu.SetActive(true);
        CartesianMenu.SetActive(false);
        CurrentState_JointSpace.SetActive(true);
    ...
    {
        ColorBlock colors = OneToOne_Btn.colors;

        chet = Math.Abs(chet - 1);
        if (chet == 0)
        {
            OneToONe = true;
    ...
    public void Toggle_Orthographic_View () 
    {
        bool a = Orthographic_Toggle.isOn;
    ...
    public void ShadowDropdown_ValueChanged ()
    {
        Light myLight = GameObject.Find("MyLight").GetComponent<Light>();
        int a = Shadow_Dpdwn.value;
    ...
    public void ShadowScrlBr_ValueChanged()
    {
        Light myLight = GameObject.Find("MyLight").GetComponent<Light>();
        float a = Shadow_Strength.value;
        myLight.shadowStrength = a;

    }

    public void Light_Red_Color() 
    {
        Light myLight = GameObject.Find("MyLight").GetComponent<Light>();
    ...
    public void Light_Green_Color()
    {
        Light myLight = GameObject.Find("MyLight").GetComponent<Light>();
        myLight.color = Color.green;

    }
    public void Light_White_Color()
    {
        Light myLight = GameObject.Find("MyLight").GetComponent<Light>();
    ...
    public void Light_Blue_Color()
    {
        Light myLight = GameObject.Find("MyLight").GetComponent<Light>();
        myLight.color = Color.blue;
    }

    public void Light_Yellow_Color()
    {
        Light myLight = GameObject.Find("MyLight").GetComponent<Light>();
        myLight.color = Color.yellow;
    }

    public void Light_Gray_Color()
    {
        Light myLight = GameObject.Find("MyLight").GetComponent<Light>();
    ...
    public void x_Increase_Button()
    {
        float x_new = float.Parse(x_input);
        float ir = 0.1f;
        x_new = x_new + ir;
        x_input = x_new.ToString();

        if (OneToONe)
        {
            floatX = float.Parse(x_input);
        }
    }

    public void x_Decrease_Button()
    {
        float x_new = float.Parse(x_input);
        float ir = 0.1f;
        x_new = x_new - ir;
    ...
    public void y_Decrease_Button()
    {
        float y_new = float.Parse(y_input);
        float ir = 0.1f;
        y_new = y_new - ir;
        y_input = y_new.ToString();

        if (OneToONe)
        {
            floatY = float.Parse(y_input);
        }
    }

    public void y_Increase_Button()
    {
        float y_new = float.Parse(y_input);
        float ir = 0.1f;
        y_new = y_new + ir;
    ...
    public void z_Increase_Button()
    {
        float z_new = float.Parse(z_input);
        float ir = 0.1f;
        z_new = z_new + ir;
        z_input = z_new.ToString();

        if (OneToONe)
        {
            floatZ = float.Parse(z_input);

        }
    }

    public void z_Decrease_Button()
    {
        float z_new = float.Parse(z_input);
        float ir = 0.1f;
        z_new = z_new - ir;
    ...
    public void psi_Increase_Button()
    {
        float psi_new = float.Parse(psi_input);
        float ir_d = 5.0f;
        psi_new = psi_new + ir_d;
        psi_input = psi_new.ToString();

        if (OneToONe)
        {
            floatPsi = float.Parse(psi_input);

        }
    }

    public void psi_Decrease_Button()
    {
        float psi_new = float.Parse(psi_input);
        float ir_d = 5.0f;
        psi_new = psi_new - ir_d;
    ...
    public void theta_Increase_Button()
    {
        float theta_new = float.Parse(theta_input);
        float ir_d = 5.0f;
        theta_new = theta_new + ir_d;
        theta_input = theta_new.ToString();

        if (OneToONe)
        {
            floatTheta = float.Parse(theta_input);
        }
    }

    public void theta_Decrease_Button()
    {
        float theta_new = float.Parse(theta_input);
        float ir_d = 5.0f;
        theta_new = theta_new - ir_d;
    ...
    public void phi_Increase_Button()
    {
        float phi_new = float.Parse(phi_input);
        float ir_d = 5.0f;
        phi_new = phi_new + ir_d;
        phi_input = phi_new.ToString();

        if (OneToONe)
        {
            floatPhi = float.Parse(phi_input);
        }
    }
    public void phi_Decrease_Button()
    {
        float phi_new = float.Parse(phi_input);
        float ir_d = 5.0f;
        phi_new = phi_new - ir_d;
